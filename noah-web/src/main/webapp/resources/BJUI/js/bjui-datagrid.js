...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        afterSelected   : null,      // Function - after selected method
        rowAttrs   : null      // Array - row attributes array
    }
    ...
    Datagrid.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            afterSelected: function($tr) {
                var afterSelected = options.afterSelected

                if (afterSelected) {
                    if (typeof afterSelected == 'string') afterSelected = afterSelected.toFunc()
                    if (typeof afterSelected == 'function') {
                        afterSelected.call(that,$tr)
                    }
                }
            },
            // Correct colspan
            ...
            createTrs: function(datas, refreshFlag) {
                ...
                $.each(datas, function(i, trData) {
                    ...
                    //console.log(options);
                    if(options.rowAttrs && $.isArray(options.rowAttrs)){//初始化加载行属性
                        for(var idx = 0;idx<options.rowAttrs.length;idx++){
                            if(trData[options.rowAttrs[idx]]) $tr.attr(options.rowAttrs[idx],trData[options.rowAttrs[idx]]);
                        }
                    }
                    //if(trData && trData.id) $tr.attr('id',trData.id);
    ...
    Datagrid.prototype.initEvents = function($trs) {
        ...
        $trs
            .on('click.bjui.datagrid.tr', function(e, checkbox) {
                //console.log('触发行点击事件...哈哈'+Math.random());
                ...
                if (checkbox) {
                    ...
                    //console.log('选中行...'+Math.random());
                    //console.log($(this).html());
                    that.tools.afterSelected($tr);//调用选择后事件
