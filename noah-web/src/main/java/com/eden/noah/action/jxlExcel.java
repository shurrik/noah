...
//			navCellView.setSize(50);
	...
	public static int exportToExcel(HttpServletResponse response,
			List<Map<String, Object>> objData, String sheetName,
			List<String> columns,String[] sizeArr) {
		int flag = 0;
		// 声明工作簿jxl.write.WritableWorkbook
		WritableWorkbook wwb;
		try {
			...
			if(sizeArr!=null)
			{
				...
				{
					ws.setColumnView(i, Integer.valueOf(sizeArr[i]));
				}				
			}
				...
				if (objData != null && objData.size() > 0) {
					...
					for (int i = 0; i < objData.size(); i++) {
						...
						for (Object o : map.keySet()) {
							...
							String val = map.get(o)!=null?String.valueOf(map
									.get(o)):"";
							ws.addCell(new Label(j, i + 1, val));
							j++;
						}
					}
				} else {
...
/*	public static void exportexcle(HttpServletResponse response,
	...
	public static void exportexcle(HttpServletResponse response,
			String filename, List<Map<String, Object>> listData,
			String sheetName, List<String> columns,String[] sizeArr) {
		// 调用上面的方法、生成Excel文件
		response.setContentType("application/vnd.ms-excel");
		// response.setHeader("Content-Disposition",
		// "attachment;filename="+filename);
		try {
			...
			exportToExcel(response, listData, sheetName, columns,sizeArr);
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
	}	
